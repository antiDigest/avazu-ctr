...
cols = ["C1", "banner_pos", "site_category", "app_category", "device_type",
        "device_conn_type", "C14", "C15", "C16", "C17", "C18", "C19", "C20", "C21", "hour"]
...
train = pd.read_csv("train", chunksize=500000, iterator=True)
...
for chunk in train:
    ...
    y_train = chunk["click"]
    chunk = chunk[cols]
    chunk = chunk.join(pd.DataFrame([dayhour(x)
                                     for x in chunk.hour], columns=["wd", "hr"]))
    chunk.drop(["hour"], axis=1, inplace=True)
    Xcat = fh.transform(np.asarray(chunk.astype(str)))
    clf.fit(Xcat, y_train)


print "Trained all, proceeding to submission."
# Create a submission file
testcols = ["id"] + cols
X_test = pd.read_csv("test", usecols=testcols)

X_test = X_test.join(pd.DataFrame([dayhour(x)
                                   for x in X_test.hour], columns=["wd", "hr"]))
X_test.drop(["hour"], axis=1, inplace=True)

X_enc_test = fh.transform(np.asarray(X_test.astype(str)))

# y_act = pd.read_csv("test", usecols=['click'])
